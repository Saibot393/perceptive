...
class PerceptiveRollHandler {
	...
	static async onChatMessage(pMessage, pInfos, pSenderID) {
		if (game.userId == pSenderID) {
			...
			if ((!vKeyboard.downKeys.has(game.keybindings.get(cModuleName, "IgnoreRoll")[0]?.key)) ^ (game.settings.get(cModuleName, "InvertIgnoreRollKey") || game.settings.get(cModuleName, "ForceInvertIgnoreRollKey"))) {
